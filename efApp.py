...
def portfolioPerformance(weights, mean_returns, cov_matrix, risk_free_rate=0):
...
def negativeSR(weights, mean_returns, cov_matrix, risk_free_rate=0):
    pReturns, pStd = portfolioPerformance(weights, mean_returns, cov_matrix, risk_free_rate)
    return -(pReturns - risk_free_rate) / pStd

def maxSR(mean_returns, cov_matrix, risk_free_rate=0, constraint_set=(0, 1)):
    numAssets = len(mean_returns)
    args = (mean_returns, cov_matrix, risk_free_rate)
    constraints = ({'type': 'eq', 'fun': lambda x: np.sum(x) - 1})
    bounds = tuple(constraint_set for _ in range(numAssets))
    result = sc.minimize(negativeSR, numAssets * [1. / numAssets], args=args,
                         method='SLSQP', bounds=bounds, constraints=constraints)
    return result

def calculated_results(mean_returns, cov_matrix, risk_free_rate=0, borrowing_rate=0, constraint_set=(0, 1)):
    max_sr_portfolio = maxSR(mean_returns, cov_matrix, risk_free_rate)
    max_sr_returns, max_sr_std = portfolioPerformance(max_sr_portfolio['x'], mean_returns, cov_matrix, risk_free_rate)
...
def ef_graph(mean_returns, cov_matrix, risk_free_rate=0.0462, borrowing_rate=0.0, constraint_set=(0, 1)):
    max_sr_returns, max_sr_std = calculated_results(mean_returns, cov_matrix, risk_free_rate, borrowing_rate, constraint_set)

    manual_weights = np.array([float(w) for w in input("Enter the portfolio weights (comma separated, e.g., '0.6,0.4'): ").split(',')])
    manual_return, manual_volatility = portfolioPerformance(manual_weights, mean_returns, cov_matrix)
